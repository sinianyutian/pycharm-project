...
# H = Dense(nch , init='he_normal')(g_input)
# H = Dense(nch * 4 * 4, init='he_normal')(g_input)
# H = BatchNormalization(mode=0)(H)
# H = Activation('relu')(H)
# H = Reshape([nch, 4, 4])(H)
#
# H = UpSampling2D(size=(4, 4))(H)
# H = Convolution2D(nch / 2, 4, 4, border_mode='same', init='he_uniform')(H)
# H = BatchNormalization(mode=0)(H)
# H = Activation('relu')(H)
#
...
# H = Activation('relu')(H)
#
# H = Convolution2D(nch / 4, 4, 4, border_mode='same', init='he_uniform')(H)
# H = BatchNormalization(mode=0)(H)
# H = Activation('relu')(H)
# H = Convolution2D(3, 1, 1, border_mode='same', init='he_uniform')(H)
# g_V = Activation('sigmoid')(H)
# g_V = cifar_gan()(g_input)
g_V = generator_model(xdim=2,ydim=2)(g_input)
...
# H = Convolution2D(128, 4, 4, subsample=(2, 2), border_mode='same', activation='relu')(d_input)
# H = LeakyReLU(0.2)(H)
# H = Dropout(dropout_rate)(H)
# H = Convolution2D(256, 4, 4, subsample=(2, 2), border_mode='same', activation='relu')(d_input)
# H = LeakyReLU(0.2)(H)
# H = Dropout(dropout_rate)(H)
# H = Convolution2D(512, 4, 4, subsample=(2, 2), border_mode='same', activation='relu')(H)
# H = LeakyReLU(0.2)(H)
# H = Dropout(dropout_rate)(H)
# H = Convolution2D(512, 4, 4, subsample=(2, 2), border_mode='same', activation='relu')(H)
# H = LeakyReLU(0.2)(H)
...
d_V = discriminator_model()(d_input)
...
train_for_n(nb_epoch=3000, plt_frq=200, BATCH_SIZE=32)
...
def plot_real(n_ex=16, dim=(4, 4), figsize=(10, 10)):
    ...
    generated_images = X_train[idx, :, :, :].copy()
    generated_images = generated_images.transpose([0, 2, 3, 1])
    plt.figure(figsize=figsize)
    for i in range(generated_images.shape[0]):
        plt.subplot(dim[0], dim[1], i + 1)
