        ...
        try:
            ...
            if is_test:
                for indexabc in range(5):
                    texnet.load_weights('style-transfer-epoch.019-35681371867.4286.hdf5')
                    ...
                    draw_images_pair(img_test, label_pred,
                                     batch_size=5, is_save=False, prefix='st-', index_pro=indexabc)
                exit()

            checkpointer = ModelCheckpoint(filepath=weight_path + '-epoch.{epoch:03d}-{loss:.4f}.hdf5',
                                           monitor='loss', verbose=1, period=20)
            ...
            texnet.fit_generator(generator=my_gen_train, steps_per_epoch=int(data_total_len / batch_size),
                                 ...
                                 callbacks=[checkpointer, tensorboard], verbose=1, workers=2)
