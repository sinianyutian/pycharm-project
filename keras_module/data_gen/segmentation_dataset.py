...
from ..data_draw.color_util import labelcolormap


class SegmentationDatasetBase(object):
    label_names = None
    mean_bgr = None
    ...
    def visualize_example(self, i):
        ...
        cmap = labelcolormap(len(self.label_names))
        ignore_mask = [label == -1]
        label[ignore_mask] = 0
    ...
    def visualize_pairs(self, datum, label):
        img = self.datum_to_img(datum)
        cmap = labelcolormap(len(self.label_names))
        ignore_mask = [label == -1]
        label[ignore_mask] = 0
