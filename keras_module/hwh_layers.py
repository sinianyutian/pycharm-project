...
class InstanceNormalization(Layer):
    ...
    def call(self, x, mask=None):
        ...
        std = K.sqrt(K.var(x, axis=axis, keepdims=True) + self.epsilon)
        x_normed_batch = (x - m) / (std + self.epsilon)
...
class AdaptiveInstanceNormalization(Layer):
    ...
    def call(self, x, mask=None):
        ...
        target_x = x[1]
        axis = [0, self.axis]
        m = K.mean(content_x, axis=axis, keepdims=True)
        std = K.sqrt(K.var(content_x, axis=self.axis, keepdims=True) + self.epsilon)
        x_normed_batch = (content_x - m) / (std + self.epsilon)

        m = K.mean(target_x, axis=axis, keepdims=True)
        std = K.sqrt(K.var(target_x, axis=self.axis, keepdims=True) + self.epsilon)

        x_normed = x_normed_batch * std + m
